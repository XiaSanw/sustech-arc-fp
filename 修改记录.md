...
**PFBlindFlatDemoEnvCfg类（第273-275行）**：
...
| 项目 | 修改前 | 修改后 | 说明 |
|------|--------|--------|------|
| `push_robot.interval` | (2.0, 3.0)s | **(4.0, 6.0)s** | 匹配训练环境 |
| `push_robot.probability` | 0.9 (90%) | **0.6 (60%)** | 匹配训练环境 |
| `push_robot.force` | 300N | **300N** | 保持不变（继承自训练配置） |
| 60秒内预期推力 | 约22次 | **约7次** | 降低至训练频率 |
...
- ✅ 偶尔被推倒属于正常（300N推力对10kg机器人仍是中等强度）
- ✅ 展示机器人真实的抗干扰能力，而非超出训练范围的极限测试
